...
__attribute__((aligned(16))) uint8_t sub_0010_arena[6144];
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0010_fast_scratch[6144];
...
int sub_0010_invoke(bool clean_outputs) {
  ...
  if (clean_outputs) {
    memset(sub_0010_arena + 1024, 0, 1024);
  }
  base_addrs[5] = (uint64_t)(uintptr_t) (sub_0010_arena+1024);
  base_addrs_size[5] = 1024;

  // Buffer output_tensor_2 with size 1024 and address: 2048
  if (clean_outputs) {
    memset(sub_0010_arena + 2048, 0, 1024);
  }
  base_addrs[6] = (uint64_t)(uintptr_t) (sub_0010_arena+2048);
