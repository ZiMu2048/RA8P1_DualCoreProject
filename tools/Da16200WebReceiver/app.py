...
<head>
  ...
  <style>
    :root{color-scheme:light;--bg:#ffffff;--hero:#e5f0fb;--brand:#2a289d;--brand-deep:#1c1b6e;--panel:#ffffff;--line:#d8dee8;--text:#1c1c1c;--muted:#5f6672;--green:#16835b;--red:#d83a52}
    *{box-sizing:border-box}body{margin:0;padding-top:106px;background:linear-gradient(to bottom,var(--brand-deep) 0 34px,var(--brand-deep) 34px,var(--brand) 34px,var(--brand) 106px,var(--hero) 106px,var(--hero) 376px,var(--bg) 376px 100%);color:var(--text);font-family:"Segoe UI","Microsoft YaHei",sans-serif;min-height:100vh}
    .site-brand{position:absolute;top:34px;left:0;right:0;height:72px;max-width:1403px;margin:auto;padding:0 52px;display:flex;align-items:center;color:#fff;font-family:"DM Sans","Microsoft YaHei",Arial,sans-serif;font-size:36px;line-height:44px;font-weight:700;letter-spacing:0;z-index:1}
    header,main{max-width:1180px;margin:auto;padding:28px 24px}header{min-height:210px;display:flex;align-items:center;justify-content:space-between;gap:20px}.eyebrow{color:var(--brand);font-size:12px;letter-spacing:.18em;font-weight:800}h1{margin:6px 0}.muted{color:var(--muted)}
    .badge{padding:10px 14px;border:1px solid #c4cce0;border-radius:999px;background:rgba(255,255,255,.82);font-weight:700;box-shadow:0 4px 14px rgba(28,27,110,.08)}.badge::before{content:"";display:inline-block;width:10px;height:10px;margin-right:9px;border-radius:50%;background:var(--red)}.badge.online::before{background:var(--green)}
    main{padding-top:8px}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:16px}.card{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:18px;box-shadow:0 8px 24px rgba(28,28,28,.07)}.stat{grid-column:span 3;border-top:3px solid var(--brand)}.stat b{display:block;font-size:24px;margin-top:8px;color:var(--brand-deep)}.preview{grid-column:span 5}.detail{grid-column:span 7}.history{grid-column:span 12}.preview img{width:100%;height:300px;object-fit:contain;background:#f3f6f9;border:1px solid var(--line);border-radius:4px}.gallery{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:14px}.item{border:1px solid var(--line);border-radius:4px;overflow:hidden;background:#ffffff}.item img{width:100%;aspect-ratio:1;object-fit:cover;display:block}.meta{padding:10px;font-size:13px}.event{padding:9px 0;border-bottom:1px solid var(--line)}
    @media(max-width:820px){.site-brand{font-size:28px;line-height:32px}h1{font-size:26px}.stat{grid-column:span 6}.preview,.detail{grid-column:span 12}}@media(max-width:520px){.site-brand{padding:0 24px;font-size:22px;line-height:26px}h1{font-size:20px}.stat{grid-column:span 12}header{align-items:flex-start;flex-direction:column}}
  </style>
</head>
<body>
  <div class="site-brand">基于RA8P1的光伏板负压吸附智能巡检系统</div>
  <header><div><h1>异常图像实时监控</h1><div class="muted">置信度超过阈值后自动接收并归档</div></div><div class="badge" id="badge">等待设备</div></header>
  <main><div class="grid">
